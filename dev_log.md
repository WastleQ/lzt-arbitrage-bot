...
## [2026-09-05] Обязательный Full Access / Родная почта (v0.3.4)
* **Фильтрация безпочтовых и неперепривязываемых аккаунтов (`require_full_access`)** — бот теперь автоматически отсеивает лоты, где почта не меняется, отсутствует полный доступ (`mail_access: false`) или в тексте есть фразы вроде "без почты", "почта не меняется", гарантируя, что купленный аккаунт перейдет полностью в собственность владельца.
* **Тестирование** — все 36 тестов (`pytest`) и линтер (`ruff`) успешно пройдены.

## [2026-09-05] Переключатели категорий в настройках и исправление TelegramBadRequest (v0.3.3)
* **Интерактивные переключатели категорий в меню Настроек (`/settings`)** — теперь прямо в Telegram можно в один клик включать и отключать парсинг Minecraft, Brawl Stars и Valorant с визуальной индикацией (🟩/🟥).
* **Исправление ошибки `TelegramBadRequest`** — вызов `callback.answer()` перенесен в самое начало всех callback-хендлеров для предотвращения устаревания query ID при выполнении запросов к API.
* **Тестирование** — все 36 тестов (`pytest`) и линтер (`ruff`) успешно пройдены.
* **Модуль `MarketCalibrator` (`src/utils/calibrator.py`)** — запрашивает свежие активные лоты с LZT Market API, вычисляет актуальные медианные цены и автоматически пересчитывает базовые цены и коэффициенты в `categories.yaml`.
* **Команда `/calibrate` в Telegram** — позволяет администратору в один клик обновить рыночные веса и базовые цены бота на основе текущей ситуации на маркете.
* **Тестирование** — добавлено покрытие тестами (`tests/test_calibrator.py`), всего 36/36 тестов пройдены успешно.
* **Интеграция структурированных полей API (`raw_data`)** — оценщики Minecraft и Brawl Stars теперь в первую очередь используют точные параметры из ответа LZT API (ранги, кубки, гиперзаряды, плащи, дни неактивности) вместо слепого парсинга заголовков.
* **Многоуровневая оценка** — гибридный подход: при отсутствии API-полей срабатывает продвинутый regex-анализ заголовков и описаний с учетом динамических весов из `categories.yaml`.
* **Тестирование** — все тесты и линтер прошли успешно.
* **Строгая фильтрация по стоп-словам (`exclude_words`)** — лоты, содержащие запрещенные слова (например, "бан", "восстановлен", "откат") в названии или описании, теперь автоматически отклоняются движком арбитража (`evaluate_item` возвращает `None`).
* **Модуль авто-перепродажи (Auto-Relist)** — при успешной покупке аккаунта через `fast_buy` бот автоматически выставляет лот обратно на маркет LZT с настраиваемой наценкой (`relist_markup_percent`, по умолчанию +30%).
* **Экспорт сделок в CSV (`/export`)** — добавлена команда для администратора, генерирующая CSV-файл со всеми совершенными сделками и историей покупок.
* **Docker & Docker Compose** — добавлены `Dockerfile` и `docker-compose.yml` для простого развертывания на любом VPS одной командой.
* **Поддержка прокси** — добавлена настройка `proxy_url` в конфигурации и клиенте LZT API.
* **Тесты и качество кода** — все 34 модульных теста (`pytest`) успешно пройдены, линтер `ruff check` не находит ошибок.
